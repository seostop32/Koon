   ...
   // 사용자 데이터 불러오기 (아바타, 코인)
    ...
    const handleConfirm = async () => {
        try {
          ...
          console.log("🎯 로그인 유저 성별:", profile.gender);
...
console.log("📌 현재 settings 상태:", settings);
      const handleFindPartner = async () => {
        ...
        console.log("🔑 로그인 유저 성별:", profileData.gender);
        ...
        // 여기서 genderToFilter 계산
        const genderToFilter = gender && gender.trim() !== ''
          ? gender.trim()
          : (profileData.gender.trim().toLowerCase() === '남성' ? '여성' : '남성');

        // gender 컬럼도 lowercase 비교 안전하게 하려면 ilike 사용
        query = query.ilike('gender', genderToFilter);          
