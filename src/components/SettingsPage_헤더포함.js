   ...
   // 사용자 데이터 불러오기 (아바타, 코인)
    ...
    const handleConfirm = async () => {
      try {
        ...
        // ✅ 코인 차감 시도
        const success = await deductCoinRpc(user.id, 'change_setting'); // ← 여기 변경!
        if (!success) return;
...
console.log("🔍 필터 결과:", data);
  ...
  function Modal({ title, options, selected = [], onToggle, onSelect, onClose, multiple = false }) {
   ...
   return (
    <div style={{ maxWidth: 600, margin: '40px auto 40px', padding: 10, fontFamily: 'Arial, sans-serif' }}>
    {/* 화면 전체 너비 헤더 */}
    <header
        style={{
          width: '100vw',
          ...
          backgroundColor: '#fff',
          zIndex: 1000,
          boxSizing: 'border-box', // 박스 크기 포함 조정
        }}
      >
        {/* 아바타 */}
        <img
        ...
        <div
          style={{
            ...
            alignItems: 'center',
            gap: 4,
            marginRight: 12,  // 오른쪽 여백 좀 넉넉하게 줌
            minWidth: 60,     // 최소 넓이 지정해서 잘리지 않게
