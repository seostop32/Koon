    ...
    <div style={styles.container}>
      ...
      <div style={styles.messagesArea}>
        {messages.map((msg, index) => {
          ...
          return (
            <div key={msg.id || `msg-${index}`} style={{ display: 'flex', flexDirection: isMine ? 'row-reverse' : 'row', alignItems: 'flex-end', marginBottom: '8px' }}>
              {/* 메시지 말풍선 */}
              <div
                style={{
                  maxWidth: '50%',
              ...
              >
                {/* 메시지 내용 부분 */}
                {msg.type === 'image' ? (
                  // 이미지: 말풍선 없이 바로 이미지 출력
                  <img
                    src={msg.content}
                    alt={msg.name || 'image'}
                    style={{
                      maxWidth: '150px',
                      borderRadius: '8px', // 둥근 모서리만 살짝 줘서 깔끔하게
                      cursor: 'pointer',
                      objectFit: 'cover',
                      display: 'block',
                    }}
                  ...
                  >
                    ...
                    {(!msg.type || msg.type === 'text') && <span>{msg.content}</span>}
                  </div>
                )}
                ...
                >
                  {isMine && msg.read_at && <span style={{ fontSize: 10, color: '#999', marginBottom: 2 }}>읽음</span>}
